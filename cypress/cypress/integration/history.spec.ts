<<<<<<< HEAD:cypress/cypress/integration/history.spec.ts
/*
...
describe("Chat History (Video Mentors)", () => {
  ...
  it("Answers can be toggled open to see the transcript of the response", () => {
    ...
    cy.get("[data-cy=history-chat]").within(($hc) => {
      cy.get("[data-cy=chat-thread]").within(($hc) => {
        ...
        cy.get("[data-cy=chat-msg-7]").scrollIntoView().should("be.visible");
  ...
  it("Question's answers can be toggled individually", () => {
    ...
    cy.get("[data-cy=history-chat]").within(($hc) => {
      cy.get("[data-cy=chat-thread]").within(($hc) => {
        ...
        cy.get("[data-cy=chat-msg-10]").should("be.visible");
        cy.get("[data-cy=chat-msg-11]").should("be.visible");
      });
    });
  });
  ...
  it("If switch is to 'Hide/Show' then when new answer arrives it is open at the bottom and all answers manually opened before it are left in their prior open/closed position", () => {
    ...
    cy.get("[data-cy=history-chat]").within(($hc) => {
      cy.get("[data-cy=chat-thread]").within(($hc) => {
        // visible answers
        cy.get("[data-cy=chat-msg-1]").scrollIntoView().should("be.visible");
        cy.get("[data-cy=chat-msg-2]").scrollIntoView().should("be.visible");
      });
    });
    ...
    cy.get("[data-cy=history-chat]").within(($hc) => {
      cy.get("[data-cy=chat-thread]").within(($hc) => {
        ...
        cy.get("[data-cy=chat-msg-16]").scrollIntoView().should("be.visible");
        cy.get("[data-cy=chat-msg-17]").scrollIntoView().should("be.visible");
      });
    });
  });
});
...
>>>>>>> (incomplete): Video mentor answer can display a link that opens a webpage tab:cypress/cypress/integration/history.spec copy.ts
